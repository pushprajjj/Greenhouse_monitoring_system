...
//Analog value moisture sensor ka max 1023 hoga lekin agar without capacitor se filter kiye signal read krna ho to body ya kisi tarah ka static charge se value upar ja sakti h jisse percentage v 100+ jayega isliye ye krna jaruri h
  ...
  if (Firebase.ready() && signupOK) {
    ...
    if (!Firebase.RTDB.setInt(&fbdo, "/IistBiharData/GreenhouseMonitoringSystem/humidity", humidity)) {
      Serial.println("Failed to update humidity to Firebase");
    }

   
    if (!Firebase.RTDB.setInt(&fbdo, "/IistBiharData/GreenhouseMonitoringSystem/temp", temperature)) {
      Serial.println("Failed to update temperature to Firebase");
    }
    
    if (!Firebase.RTDB.setInt(&fbdo, "/IistBiharData/GreenhouseMonitoringSystem/MoistureValue", soilMoisture)) {
  ...
  if (Firebase.ready() && signupOK) {
    if (Firebase.RTDB.getInt(&fbdo, "/IistBiharData/GreenhouseMonitoringSystem/relay2")) {
    ...
    if (Firebase.RTDB.getInt(&fbdo, "/IistBiharData/GreenhouseMonitoringSystem/relay3")) {
      int status = fbdo.intData();
      if (status == 1) {
    ...
    if (Firebase.RTDB.getInt(&fbdo, "/IistBiharData/GreenhouseMonitoringSystem/relay4")) {
      int status = fbdo.intData();
      if (status == 1) {
